...
correo `ciss-bienestar.org`. Escrito para retomarlo en frío.
...
6. Crear la cuenta del panel en **Authentication → Users** (con *Auto Confirm
   ...
   values ('<uuid>', '<persona>@ciss-bienestar.org', '<Nombre>', 'superadmin');
   ```

### 2.4 · Correo, con el dominio nuevo

1. Cuenta de Resend con el correo institucional.
2. **Domains → Add domain** → `ciss-bienestar.org`.
...
4. En Vercel:
   ...
   CORREO_CONTACTO=congreso@ciss-bienestar.org
   ```
...
`congreso.ciss-bienestar.org`. En Vercel, *Settings → Domains → Add*, y crear en
...
2. **Dirección del sitio**: ¿`congreso.ciss-bienestar.org`? ¿otra?
3. **Buzón del congreso**: ¿`congreso@ciss-bienestar.org`? Es el que verán los
