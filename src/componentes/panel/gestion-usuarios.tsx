...
export function GestionUsuarios({ usuarios }: { usuarios: UsuarioFila[] }) {
  ...
  async function actualizar(id: string, cambios: Partial<UsuarioFila>) {
    ...
    const { error } = await supabase.from('usuarios_panel').update(cambios).eq('id', id);
    setMensaje(error ? error.message : t.estados.guardado);
    ...
    <div className="p-4 sm:p-6">
      <header className="mb-5">
        ...
        <p className="ayuda !mt-1">
          Las cuentas se crean desde Supabase Auth; aquí se asigna el rol y se activa o desactiva
          el acceso al panel. Cada cambio queda asentado en la auditoría.
        </p>
      </header>
